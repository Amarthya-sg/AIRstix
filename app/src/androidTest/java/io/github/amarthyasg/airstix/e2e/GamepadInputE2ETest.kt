...
class GamepadInputE2ETest {
    ...
    fun testButtonInputs() {
        ...
        println("Waiting for connection success and redirect to Main Menu...")
        ...
        println("Tapping Resume button to enter Gamepad...")
    ...
    @Test
    fun testConnectionLostAndRecovery() {
        val port = TestGamepadServer.getPort().toString()
        println("Connecting to port: $port")
        ...
        // Stop the server (and close active client socket)
        println("Stopping server to simulate disconnect")
        TestGamepadServer.stop()



        // Verify we are taken to the ConnectionLostScreen
        println("Waiting for ConnectionLostScreen...")
        composeTestRule.waitUntil(10000) {
            try {
        ...
        // Verify the screen components are present
        composeTestRule.onNodeWithText("reconnect").assertIsDisplayed()
        composeTestRule.onNodeWithText("Main Menu").assertIsDisplayed()

        // Tap Main Menu and verify we are back at the Main Menu
        println("Tapping Main Menu")
