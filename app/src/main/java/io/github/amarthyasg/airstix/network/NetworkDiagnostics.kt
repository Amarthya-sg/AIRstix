...
class NetworkDiagnostics(private val context: Context) {
     ...
     */
    suspend fun checkNetworkConnectivity(): Boolean = withContext(Dispatchers.IO) {
            ...
            for (intf in interfaces) {
                ...
                for (addr in addrs) {
                    if (addr != null && !addr.isLoopbackAddress) {
                        ...
                        if (isIPv4) return@withContext sAddr
                    }
                }
            }
        } catch (e: Exception) {
     ...
     */
    suspend fun isSameSubnet(ip1: String, ip2: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            if (b1.size != 4 || b2.size != 4) return@withContext false

            val prefix = NetworkInterface.getByInetAddress(addr1)
                ?.interfaceAddresses?.firstOrNull { it.address == addr1 }
                ?.networkPrefixLength?.toInt() ?: return@withContext false
